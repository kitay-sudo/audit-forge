...
> *Скачай `https://raw.githubusercontent.com/kitay-sudo/audit-forge/main/AUDITFORGE.md` и выполни его
> целиком на ЭТОМ репозитории. Создай в корне проекта папку `auditforge/` и сложи туда полный отчёт и по
...
```bash
git apply --check auditforge/fixes/CRIT-01-*.diff   # проверить
git apply auditforge/fixes/CRIT-01-<slug>.diff       # применить один
```

> Если агент **не умеет качать URL** — откройте raw-ссылку сами, вставьте текст в чат и добавьте:
> *«Выполни этот плейбук на репозитории, результат — в `auditforge/`.»*
...
1. Скопируйте [`AUDITFORGE.md`](./AUDITFORGE.md) и [`REPORT_TEMPLATE.md`](./REPORT_TEMPLATE.md) в корень
   проекта.
2. Дайте агенту промпт из §24 плейбука (**Quick-start prompt**) — он создаст ту же папку `auditforge/`.
...
Весь результат складывается в одну папку **`auditforge/`** в корне проверяемого проекта (остальной
